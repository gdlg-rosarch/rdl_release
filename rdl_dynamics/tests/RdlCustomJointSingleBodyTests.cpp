...
TEST_F (CustomJointSingleBodyFixture, UpdateKinematics)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        EXPECT_TRUE(unit_test_utils::checkMatrix3dEpsilonClose(reference_model.at(idx).bodyFrames[reference_body_id.at(idx)]->getInverseTransformToRoot().E, custom_model.at(idx).bodyFrames[custom_body_id.at(idx)]->getInverseTransformToRoot().E, unit_test_utils::TEST_PREC));

        EXPECT_TRUE(unit_test_utils::checkSpatialVectorsEpsilonClose(reference_model.at(idx).v[reference_body_id.at(idx)], custom_model.at(idx).v[custom_body_id.at(idx)], unit_test_utils::TEST_PREC));

        EXPECT_TRUE(unit_test_utils::checkSpatialVectorsEpsilonClose(reference_model.at(idx).a[reference_body_id.at(idx)], custom_model.at(idx).a[custom_body_id.at(idx)], unit_test_utils::TEST_PREC));
    }
}
...
TEST_F (CustomJointSingleBodyFixture, Jacobians)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        for(int i = 0; i < GDotcusPt.rows(); ++i)
        {
            ...
            {
                EXPECT_NEAR (GDotrefPt(i, j), GDotcusPt(i, j), TEST_PREC);
            }
        }
...
TEST_F (CustomJointSingleBodyFixture, CentroidalMomentum)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        EXPECT_TRUE(unit_test_utils::checkMatrixNdEpsilonClose(ADotref,ADotcus,unit_test_utils::TEST_PREC));
    }
}
...
TEST_F (CustomJointSingleBodyFixture, InverseDynamics)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        EXPECT_TRUE(unit_test_utils::checkVectorNdEpsilonClose(tauRef, tauCus, unit_test_utils::TEST_PREC));
    }
...
TEST_F (CustomJointSingleBodyFixture, CompositeRigidBodyAlgorithm)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        EXPECT_TRUE(unit_test_utils::checkVectorNdEpsilonClose(qddot_crba_ref, qddot_crba_cus, unit_test_utils::TEST_PREC));
    }
}
...
TEST_F (CustomJointSingleBodyFixture, ForwardDynamics)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        EXPECT_TRUE(unit_test_utils::checkVectorNdEpsilonClose(qddotRef, qddotCus, unit_test_utils::TEST_PREC));
    }
...
TEST_F (CustomJointSingleBodyFixture, CalcMInvTimestau)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        //check.
        EXPECT_TRUE(unit_test_utils::checkVectorNdEpsilonClose(qddot_minv_ref, qddot_minv_cus, unit_test_utils::TEST_PREC));
    }
...
TEST_F (CustomJointSingleBodyFixture, ForwardDynamicsContactsKokkevis)
{
    ...
    for(int idx = 0; idx < NUMBER_OF_MODELS; ++idx)
    {
        ...
        if(dof > 1)
        {
            ...
            EXPECT_TRUE(unit_test_utils::checkVectorNdEpsilonClose(qdot_plus_ref, qdot_plus_cus, unit_test_utils::TEST_PREC));

            EXPECT_TRUE(unit_test_utils::checkVectorNdEpsilonClose(qddot_ref, qddot_cus, unit_test_utils::TEST_PREC));
        }
    }
