...
namespace RobotDynamics
{
    ...
    RDL_DLLAPI void jcalc(Model &model, unsigned int joint_id, const VectorNd &q, const VectorNd &qdot)
    {
        ...
        else if(model.mJoints[joint_id].mJointType == JointTypeEulerXYZ)
        {
            ...
            model.X_J[joint_id].E = Matrix3d(c2 * c1, s2 * c0 + c2 * s1 * s0, s2 * s0 - c2 * s1 * c0, -s2 * c1, c2 * c0 - s2 * s1 * s0, c2 * s0 + s2 * s1 * c0, s1, -c1 * s0, c1 * c0);
        ...
        else if(model.mJoints[joint_id].mJointType == JointTypeEulerYXZ)
        {
            ...
            model.X_J[joint_id].E = Matrix3d(c2 * c0 + s2 * s1 * s0, s2 * c1, -c2 * s0 + s2 * s1 * c0, -s2 * c0 + c2 * s1 * s0, c2 * c1, s2 * s0 + c2 * s1 * c0, c1 * s0, -s1, c1 * c0);
...
//            model.c_J[joint_id].set(c2 * c1 * qdot2 * qdot0 - s2 * s1 * qdot1 * qdot0 - s2 * qdot2 * qdot1, -s2 * c1 * qdot2 * qdot0 - c2 * s1 * qdot1 * qdot0 - c2 * qdot2 * qdot1, -c1 * qdot1 * qdot0, 0., 0., 0.);
            model.c_J[joint_id] = model.multdof3_S_o[joint_id] * qdotv;//.set(c2 * c1 * qdot2 * qdot0 - s2 * s1 * qdot1 * qdot0 - s2 * qdot2 * qdot1, -s2 * c1 * qdot2 * qdot0 - c2 * s1 * qdot1 * qdot0 - c2 * qdot2 * qdot1, -c1 * qdot1 * qdot0, 0., 0., 0.);
        }
    ...
    RDL_DLLAPI Math::SpatialTransform jcalc_XJ(Model &model, unsigned int joint_id, const Math::VectorNd &q)
    {
        // exception if we calculate it for the root body
        assert (joint_id > 0);
        ...
        if(model.mJoints[joint_id].mDoFCount == 1 && model.mJoints[joint_id].mJointType != JointTypeCustom)
        {
            ...
            {
                return Xrot(q[model.mJoints[joint_id].q_index], Vector3d(model.mJoints[joint_id].mJointAxes[0][0], model.mJoints[joint_id].mJointAxes[0][1], model.mJoints[joint_id].mJointAxes[0][2]));
            }
            else if(model.mJoints[joint_id].mJointType == JointTypePrismatic)
            {
                return Xtrans(Vector3d(model.mJoints[joint_id].mJointAxes[0][3] * q[model.mJoints[joint_id].q_index], model.mJoints[joint_id].mJointAxes[0][4] * q[model.mJoints[joint_id].q_index], model.mJoints[joint_id].mJointAxes[0][5] * q[model.mJoints[joint_id].q_index]));
            }
        }
        else
        {
    ...
    RDL_DLLAPI void jcalc_X_lambda_S(Model &model, unsigned int joint_id, const VectorNd &q)
    {
        // exception if we calculate it for the root body
        assert (joint_id > 0);
        ...
        {
            model.bodyFrames[joint_id]->setTransformFromParent(Xroty(q[model.mJoints[joint_id].q_index]) * model.X_T[joint_id]);
            model.bodyFrames[joint_id]->update();
            model.bodyCenteredFrames[joint_id]->update();
            model.S[joint_id] = model.mJoints[joint_id].mJointAxes[0];
        }
        ...
        {
            model.bodyFrames[joint_id]->setTransformFromParent(Xrotz(q[model.mJoints[joint_id].q_index]) * model.X_T[joint_id]);
            model.bodyFrames[joint_id]->update();
            model.bodyCenteredFrames[joint_id]->update();
            model.S[joint_id] = model.mJoints[joint_id].mJointAxes[0];
        }
        ...
        {
            model.bodyFrames[joint_id]->setTransformFromParent(jcalc_XJ(model, joint_id, q) * model.X_T[joint_id]);
            model.bodyFrames[joint_id]->update();
            model.bodyCenteredFrames[joint_id]->update();
            // Set the joint axis
            model.S[joint_id] = model.mJoints[joint_id].mJointAxes[0];
        ...
        {
            model.bodyFrames[joint_id]->setTransformFromParent(SpatialTransform(model.GetQuaternion(joint_id, q).toMatrix(), Vector3d(0., 0., 0.)) * model.X_T[joint_id]);
            model.bodyFrames[joint_id]->update();
            model.bodyCenteredFrames[joint_id]->update();
        ...
        else if(model.mJoints[joint_id].mJointType == JointTypeEulerXYZ)
        {
            ...
            model.bodyFrames[joint_id]->setTransformFromParent(SpatialTransform(Matrix3d(c2 * c1, s2 * c0 + c2 * s1 * s0, s2 * s0 - c2 * s1 * c0, -s2 * c1, c2 * c0 - s2 * s1 * s0, c2 * s0 + s2 * s1 * c0, s1, -c1 * s0, c1 * c0), Vector3d(0., 0., 0.)) * model.X_T[joint_id]);
            model.bodyFrames[joint_id]->update();
            model.bodyCenteredFrames[joint_id]->update();
        ...
        else if(model.mJoints[joint_id].mJointType == JointTypeEulerYXZ)
        {
            ...
            model.bodyFrames[joint_id]->setTransformFromParent(SpatialTransform(Matrix3d(c2 * c0 + s2 * s1 * s0, s2 * c1, -c2 * s0 + s2 * s1 * c0, -s2 * c0 + c2 * s1 * s0, c2 * c1, s2 * s0 + c2 * s1 * c0, c1 * s0, -s1, c1 * c0), Vector3d(0., 0., 0.)) * model.X_T[joint_id]);
            model.bodyFrames[joint_id]->update();
            model.bodyCenteredFrames[joint_id]->update();
        ...
        else if(model.mJoints[joint_id].mJointType == JointTypeTranslationXYZ)
        {
            ...
            model.bodyFrames[joint_id]->setTransformFromParent(SpatialTransform(Matrix3d::Identity(3, 3), Vector3d(q0, q1, q2)) * model.X_T[joint_id]);
            model.bodyFrames[joint_id]->update();
            model.bodyCenteredFrames[joint_id]->update();
