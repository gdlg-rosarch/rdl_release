...
namespace RobotDynamics
{
     ...
     *
     * The Robot Dynamics Library supports a multitude of joints:
     * revolute, planar, fixed, singularity-free spherical joints and joints
     * with multiple degrees of freedom in any combinations.
     *
     * Fixed joints do not cause any overhead in RDL as the bodies that are
     * rigidly connected are merged into a single body. For details see \ref joint_models_fixed.
     *
     ...
     * simplifies the required algebra and also code branching in RDL. A
     ...
     * This vector follows the following convention: \f[ (r_x, r_y, r_z, t_x, t_y, t_z) \f]
     *
     ...
     *
     * RDL has highly efficient implementations for the following three degree
     * of freedom joints:
     * <ul>
     ...
     *
     * RDL has a special joint type for floating-base systems that uses the
     ...
     *
     * For this case RDL has the special joint type
     * RobotDynamics::JointTypeSpherical. When using this joint type the
     ...
     * RDL stores the Quaternions in \f$\mathbf{q}\f$ such that the 4th component of
     ...
     *\\
       \mathbf{\bar{q}} &=& ( \dot{q}_{tx}, \omega_{1,x}, \omega_{1,y}, \omega_{1,z}, \dot{q}_{ty}, \omega_{2,x}, \omega_{2,y},
     *\\*\omega_{2,z} )^T \\
       \mathbf{\bar{\bar{q}}} &=& ( \ddot{q}_{tx}, \dot{\omega}_{1,x}, \dot{\omega}_{1,y}, \dot{\omega}_{1,z}, \ddot{q}_{ty},
     *\\*\dot{\omega}_{2,x}, \dot{\omega}_{2,y}, \dot{\omega}_{2,z} )^T \\
    ...
    enum JointType
    {
        JointTypeUndefined = 0, JointTypeRevolute, JointTypePrismatic, JointTypeRevoluteX, JointTypeRevoluteY,
        JointTypeRevoluteZ, JointTypeSpherical,         ///< 3 DoF joint using Quaternions for joint positional variables and
                                                        // angular velocity for joint velocity variables.
        JointTypeEulerZYX,                              ///< 3 DoF joint that uses Euler ZYX convention (faster than emulated
                                                        // multi DoF joints).
        JointTypeEulerXYZ,                              ///< 3 DoF joint that uses Euler XYZ convention (faster than emulated
                                                        // multi DoF joints).
        JointTypeEulerYXZ,                              ///< 3 DoF joint that uses Euler YXZ convention (faster than emulated
                                                        // multi DoF joints).
        JointTypeTranslationXYZ, JointTypeFloatingBase, ///< A 6-DoF joint for floating-base (or freeflyer) systems.
        JointTypeFixed,                                 ///< Fixed joint which causes the inertial properties to be merged with
                                                        // the parent body.
        JointType1DoF, JointType2DoF,                   ///< Emulated 2 DoF joint.
     ...
     *
     * @note The construction of a Joint is NOT a realtime safe procedure, so be sure you are not
     * creating temporary joints at runtime. For maximum performance, you should create all joints
     * at config time and just reference those.
     *
    ...
    struct RDL_DLLAPI Joint
    {
        ...
        Joint(JointType type,
              int       degreesOfFreedom) : mJointAxes(NULL), mJointType(type), mDoFCount(0), q_index(0), custom_joint_index(-1)
        {
        ...
        Joint(const Joint& joint) : mJointType(joint.mJointType), mDoFCount(joint.mDoFCount), q_index(joint.q_index),
    ...
    /** \brief Computes all variables for a joint model and updates the body frame as well as the body's
     * center of mass frame
     *
     *	By appropriate modification of this function all types of joints can be
     *	modeled. See RobotDynamics::CustomJoint
     *
     * \param model    the rigid body model
     * \param joint_id the id of the joint we are interested in. This will be used to determine the type of joint and also the
     **entries of \f[ q, \dot{q} \f].
    ...
    /**
     * @struct CustomJoint
     * @brief CustomJoint is a struct used to create a joint with user defined parameters. This is
     * accomplished by overriding the RobotDynamics::Joint::jcalc methods that calculate each joints
     * kinematic parameters
     */
    struct RDL_DLLAPI CustomJoint
    {
        /**
         * @brief Constructor
         */
        ...
        /**
         * @deprecated Use RobotDynamics::CustomJoint::jcalc_new instead
         * @param model
         * @param joint_id
         * @param q
         * @param qdot
        ...
        unsigned int           mDoFCount; /**< Degrees of freedom of the joint */
        Math::SpatialTransform XJ;        /**< Joint state transform. Transform from parent */
        Math::MatrixNd         S;         /**< Motion subspace map */
        Math::MatrixNd         S_o;       /**< Motion subspace map */
        Math::MatrixNd         U;
        Math::MatrixNd         Dinv;
        Math::VectorNd         u;
        Math::VectorNd         d_u;
        Math::VectorNd         ndof0_vec;    /**< mDoFCount x 1 vector of zeros */
        Math::VectorNd         tmp_ndof_vec; /**< length ndof vector for intermediate calculations */
    };
}
