...
namespace RobotDynamics
{
    namespace Urdf
    {
        ...
        bool construct_model(Model *rdl_model, ModelPtr urdf_model, bool floating_base, bool verbose)
        {
...
//            while(link_stack.size() > 0)
            ...
            for(j = 0; j < joint_names.size(); j++)
            {
                ...
                if(urdf_parent->name != "base_joint" && rdl_model->mBodies.size() != 1)
                {
                ...
                //cout << "joint: " << urdf_joint->name << "\tparent = " << urdf_parent->name << " child = " << urdf_child->name << " parent_id = " << rdl_parent_id << endl;
        ...
        RDL_DLLAPI bool urdfReadFromFile(const char *filename, Model *model, bool floating_base, bool verbose)
        {
            ifstream model_file(filename);
            if(!model_file)
            {
        ...
        RDL_DLLAPI bool urdfReadFromString(const char *model_xml_string, Model *model, bool floating_base,
                bool verbose)
        {
            assert (model);
