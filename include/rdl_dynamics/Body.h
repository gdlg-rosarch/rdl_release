...
namespace RobotDynamics
{
    ...
    struct RDL_DLLAPI Body
    {
        ...
        void join(const Math::SpatialTransform& transform, const Body& other_body)
        {
            ...
            Math::RigidBodyInertia other_rbi = Math::createFromMassComInertiaC(other_body.mMass,
            ...
            Math::Matrix3d inertia_other_com_rotated_this_origin = Math::parallel_axis(inertia_other_com_rotated,
                                                                                       other_mass,
                                                                                       other_com);

            // 4. Sum the two inertias
            Math::Matrix3d inertia_summed =
                Math::Matrix3d(this_rbi.toMatrix().block<3, 3>(0, 0)) + inertia_other_com_rotated_this_origin;

            // 5. Transform the summed inertia to the new COM
            Math::Matrix3d new_inertia = inertia_summed - new_mass *Math::toTildeForm(new_com) *
                                         Math::toTildeForm(new_com).transpose();
