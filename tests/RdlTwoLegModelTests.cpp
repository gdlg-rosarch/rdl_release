...
enum ParamNames
{
    ParamSteplength = 0, ParamNameLast
};

enum PosNames
{
    PosHipPosX, PosHipPosY, PosHipRotZ, PosRightThighRotZ, PosRightShankRotZ, PosRightAnkleRotZ, PosLeftThighRotZ, PosLeftShankRotZ, PosLeftAnkleRotZ, PosNameLast
};

enum StateNames
{
    StateHipPosX, StateHipPosY, StateHipRotZ, StateRightThighRotZ, StateRightShankRotZ, StateRightAnkleRotZ, StateLeftThighRotZ, StateLeftShankRotZ, StateLeftAnkleRotZ, StateHipVelX, StateHipVelY, StateHipRotVelZ, StateRightThighRotVelZ, StateRightShankRotVelZ, StateRightAnkleRotVelZ, StateLeftThighRotVelZ, StateLeftShankRotVelZ, StateLeftAnkleRotVelZ, StateNameLast
};

enum ControlNames
{
    ControlRightThighRotZ, ControlRightKneeRotZ, ControlRightAnkleRotZ, ControlLeftThighRotZ, ControlLeftKneeRotZ, ControlLeftAnkleRotZ, ControlNameLast
};

enum SegmentLengthsNames
{
    SegmentLengthsHip = 0, SegmentLengthsThigh, SegmentLengthsShank, SegmentLengthsFootHeight, SegmentLengthsFoot, SegmentLengthsNameLast
};
...
enum JointLocations
{
    JointLocationHip = 0, JointLocationKnee, JointLocationAnkle, JointLocationLast
};
...
{
    SegmentMassHip, SegmentMassThigh, SegmentMassShank, SegmentMassFoot, SegmentMassLast
};
...
enum COMNames
{
    COMHip, COMThigh, COMShank, COMFoot, COMNameLast
};

Vector3d com_position[COMNameLast] = {Vector3d(0., 0.3469 * ModelHeight, 0.), Vector3d(0., 0.2425 * ModelHeight, 0.), Vector3d(0., 0.2529 * ModelHeight, 0.), Vector3d(0.0182 * ModelHeight, 0., 0.)};

enum RGyrationNames
{
    RGyrationHip, RGyrationThigh, RGyrationShank, RGyrationFoot, RGyrationLast
};

Vector3d rgyration[RGyrationLast] = {Vector3d(0.1981, 0.1021, 0.1848), Vector3d(0.1389, 0.0629, 0.1389), Vector3d(0.1123, 0.0454, 0.1096), Vector3d(0.0081, 0.0039, 0.0078)};
...
TEST (RbdlimTwoLegModelTests, TestForwardDynamicsContactsDirectFootmodel)
{
    ...
    EXPECT_TRUE(unit_test_utils::checkVector3dEpsilonClose(Vector3d(0., 0., 0.), contact_accel_left, TEST_PREC));

    delete model;
}

TEST (RbdlimTwoLegModelTests, TestClearContactsInertiaMatrix)
{
    Model *model = new Model;
    ...
    EXPECT_TRUE(unit_test_utils::checkVectorNdEpsilonClose(QDDot_lag, QDDot_aba, TEST_PREC * QDDot_lag.norm()));
